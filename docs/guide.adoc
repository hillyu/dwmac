...
Dwmac stores all windows and containers in a tree.
Dwmac tree tiling model is https://i3wm.org/docs/userguide.html#tree[inspired by i3].

*Definition.* Each non-leaf node is called a "Container"

WARNING: i3 has a different terminology.
"container" in i3 is the same as "node" in Dwmac.
...
. <<layouts,Layout>> (Possible values: `master-stack`)
...
The tree structure can be changed with one command:
...
In total, Dwmac provides 2 possible tiling layouts:

- `h_master_stack` horizontal master-stack (similar to dwm's master-stack)
- `v_master_stack` vertical master-stack

[#normalization]
=== Normalization

By default, Dwmac does two types of tree normalizations:
